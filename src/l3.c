...
#define STEP 2
#define GET_PLACE(p) (((p)&0x3f)/STEP)
#define SETS_TO_SAMPLE L3_SETS_PER_PAGE/STEP
...
int probets(void *pp) {
    ...
    return mend.tv_nsec - mstart.tv_nsec;
    //return rdtscp()-s;
}

int bprobets(void *pp) {
    if (pp == NULL)
    return 0;
    return probets(NEXTPTR(pp));
}
...
void l3_probe(l3pp_t l3, uint16_t *results) {
    for (int i = 0; i < l3->nmonitored; i++) {
        ...
        results[(GET_PLACE(l3->monitoredset[i]))]+= t > UINT16_MAX ? UINT16_MAX : t;
    }
}
...
void l3_probecount(l3pp_t l3, uint16_t *results) {
    for (int i = 0; i < l3->nmonitored; i++){ //Lishay: for each probed cache set 
    results[(GET_PLACE(l3->monitoredset[i]))]+=probecount(l3->monitoredhead[i]); //Lishay: count number of misses in cache 
    }
}

void l3_bprobecount(l3pp_t l3, uint16_t *results) {
    for (int i = 0; i < l3->nmonitored; i++){
    results[(GET_PLACE(l3->monitoredset[i]))]+= bprobecount(l3->monitoredhead[i]);
    }
}
...
void l3_bprobets(l3pp_t l3, uint64_t *results) {
    for (int i = 0; i < L3_SETS_PER_PAGE / STEP; i++){
        results[i] = bprobets(l3->monitoredhead_compressed[i]);
    }
}
...
int l3_repeatedprobe(l3pp_t l3, int nrecords, uint16_t *results, int slot) {
    ...
    for (int i = 0; i < nrecords; i++) {
        if (missed) {
            ...
            if (even)
            l3_probe(l3, localResults);
            else
            l3_bprobe(l3, localResults);
            even = !even;
        }
...
//                        the cache array,overallsaamples,(SLOT * 1000 * CPUMHZ)
int l3_repeatedprobecount(l3pp_t l3, int nrecords, uint16_t *results, int slot) {
    ...
    int len = SETS_TO_SAMPLE; //lishay: number of sets that will be measured in each page
    ...
    uint16_t localResults[SETS_TO_SAMPLE];
    int n=0;
    for (int i = 0; i < nrecords; i++) { //lishay: each sample it will only measure one page. - 64/step sets
        n++;
        if (missed) {
            for (int j = 0; j < len; j++)
                localResults[j] = -1;
            ...
            for (int j = 0; j < len; j++)
                localResults[j] = 0;
            //lishay: probe the page
            if (even)
                l3_probecount(l3, localResults);
            else
        ...
        if (slot > 0) {
            prev_time += slot;
            missed = slotwait(prev_time);
        } //lishay: should wait for 2ms to pass before moving on
    }
    return nrecords;
return nrecords;
}
 ...
 int l3_repeatedprobeperf(l3pp_t l3, int nrecords, uint16_t *results, int slot) {
    ...
    for (int i = 0; i < nrecords; i++) {//next timeslot place in result
        if (missed) {
            ...
            if (even)  
                l3_probeperf(l3, localResults);
            else
                l3_bprobeperf(l3, localResults);
            even = !even;
    ...
    for (int i = 0; i < nrecords; i++) {//next timeslot place in result
        if (missed) {
           ...
           for (int j = 0; j < len; j++)
            ...
            if (even)
                l3_probeperf_compressed(l3, localResults);
            else
                l3_bprobeperf_compressed(l3, localResults);
            even = !even;
...
int l3_repeatedprobets(l3pp_t l3, int nrecords, uint64_t *results, int slot) {
    ...
    int missed = 0;
    uint64_t prev_time = rdtscp64();//current time in cpu ticks##
    uint64_t localResults[SETS_TO_SAMPLE];
    ...
    for (int i = 0; i < nrecords; i++) {//next timeslot place in result
        if (missed) {
            ...
            if (even)
            l3_probets(l3, localResults);
            else
            l3_bprobets(l3, localResults);
            even = !even;
...
// squish all monitored sets into 32 data structures.
// Note: this kills the entire data structure since it overwrites the linked lists.
void l3_compress_monitored_sets(l3pp_t l3) {
  void *old_tail = NULL, *old_b_tail = NULL;
  void *new_head = NULL, *new_b_head = NULL;
  
  for (int compressed_set_index = 0; compressed_set_index < L3_SETS_PER_PAGE / STEP; compressed_set_index ++) {
    // Place a dummy value here just so it won't get stuck
    l3->monitoredhead_compressed[compressed_set_index] = l3->monitoredhead[0];
    // Find a list that maps to this set index
    for (int set_index = 0; set_index < l3->nmonitored; set_index++) {
      if (GET_PLACE(l3->monitoredset[set_index]) == compressed_set_index) {
        // If this is the first one we find, put it in our data structure
        if (new_head == NULL) {
          l3->monitoredhead_compressed[compressed_set_index] = l3->monitoredhead[set_index];
        }
        new_head = l3->monitoredhead[set_index];
        new_b_head = NEXTPTR(l3->monitoredhead[set_index]);

        // connect the current tail of the list to the head of this new list
        // (also the one that goes backwards duh)
        if (old_tail != NULL) {
          LNEXT(old_tail) = new_head;
          LNEXT(old_b_tail) = new_b_head;
        }

        // update the tail
        while (LNEXT(new_head) != l3->monitoredhead[set_index]) {
          old_tail = new_head;
          new_head = LNEXT(new_head);

          old_b_tail = new_b_head;
          new_b_head = LNEXT(new_b_head);

        }   
      }
    }
    // Connect the current tail of the list to the head of the list so it goes around and around
    // (also the backward list)
    LNEXT(old_tail) = l3->monitoredhead_compressed[compressed_set_index];
    LNEXT(old_b_tail) = NEXTPTR(l3->monitoredhead_compressed[compressed_set_index]);
    // Preparpe tof rht enxt reound
    new_head = NULL;
    old_tail = NULL;
    new_b_head = NULL;
    old_b_tail = NULL;
